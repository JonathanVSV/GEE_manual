...
Para realizar este filtro se deben escribir exactamente igual el nombre de las propiedades (por ejemplo, 'CLOUD_COVER_LAND') y en la definición del filtro se debe explicitar si se desea utilizar los valores que sean iguales (`.eq`), distintos (`.neq`), mayores (`.gt`), menores (`.lt`), mayores o igual (`.gte`) o menores o iguales(`.lte`).
\index{filterDate}
\index{filter}
\index{ee.Filter.lte}
\index{ee.Filter.eq}
...
Para ver una muestra de cómo se ve la primer imagen con y sin la máscara de nubes, primero se agregará la imagen con nubes a la pantalla de mapa (`r colorize("Fig.")` \@ref(fig:f1017)).
\index{first}
```{js, echo = T}
...
A continuación, agregamos los dos resultados a la pantalla de mapas y visualizamos el compuesto de media (`r colorize("Fig.")` \@ref(fig:f1020)).
\index{Map.addLayer}
```{js, echo = T}
...
Para después cortar las dos imágenes y mostrarlas en la pantalla de mapas (`r colorize("Fig.")` \@ref(fig:f1022)).
\index{clip}
```{js  echo = T}
// Cortar la imágenes
...
Posteriormente se agrega el resultado a la pantalla de mapas (`r colorize("Fig.")` \@ref(fig:f1023)).
