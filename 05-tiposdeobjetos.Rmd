...
Se refiere a objetos de cadenas de símbolos de tipo alfanumérico. Estos se definen como cualquier cadena de caracteres (número, letras o símbolos) que se encuentren entre un par de comillas bien sean dobles `“ ”` o sencillas `’ ’`. Por ejemplo (`r colorize("Fig.")` \@ref(fig:f52)):
...
Las claves o entradas siempre serán leídas como cadenas de texto. Se recomienda que no tengan espacios dentro de ellas, en su lugar se recomienda usar guion bajo (`_`) o la notación de "lowerCamelCase". Al hacer `print` se ordenarán alfabéticamente las claves (`r colorize("Fig.")` \@ref(fig:f55)).
...
Permite enviar un número como objeto al servidor. Además, al definir un objeto de este tipo, se puede utilizar cualquier método indicado para objetos de tipo `ee.Number` en la API (ver liga anterior; `r colorize("Fig.")` \@ref(fig:f58)).
...
Este tipo de objetos corresponden a listas del lado del servidor. Al definir un objeto de este tipo, se puede utilizar cualquier método indicado para objetos `ee.List` (ver enlace anterior; `r colorize("Fig.")` \@ref(fig:f59)).
...
```
```{r f59, echo = F, out.width="80%", fig.cap="Salida de la consola de un objeto de tipo lista del lado del servidor."}
knitr::include_graphics("Img/ej6.png")
```

#### ee.Dictionary {-}

Permite enviar un objeto como diccionario al servidor. La definición de objetos `ee.Dictionary` habilita los métodos disponibles en GEE para este tipo de objetos (ver liga anterior; `r colorize("Fig.")` \@ref(fig:f510)).
...
Por último, si se desea obtener las claves disponibles en un `ee.Dictionary` se utiliza el método `.keys`. Este método devuelve una lista. De igual forma, si se desea obtener los valores disponibles en un `ee.Dictionary` se utiliza el método `.values`, la cual devuelve una lista (`r colorize("Fig.")` \@ref(fig:f510)).
...
Esta es la forma en la que GEE trabaja con fechas. Al definir un objeto de este tipo, se puede utilizar cualquier método indicado para objetos de tipo `ee.Date` (ver enlace anterior; `r colorize("Fig.")` \@ref(fig:f511)). Hay varias formas de construir una fecha específica:
