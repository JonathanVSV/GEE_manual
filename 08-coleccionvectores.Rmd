...
```
```{r f81, echo = F, out.width="95%", fig.cap="Visualización de la colección de vectores con los límites internacionales, así como su salida en la consola."}
...
```{r f82, echo = F, out.width="95%", fig.cap="Visualización de las primeras 1000 cuencas, así como su salida en la consola."}
knitr::include_graphics("Img/ej14.png")
```

:::: {.bluebox2 data-latex=""}
::: {.awesomeblock data-latex="{5pt}{\faLightbulb}{darkblue}"}
Al usar `r colorize2("print")` con las colecciones de vectores, la consola puede mostrar un máximo de 5000 vectores. En caso que uno necesite usar `r colorize2("print")` en una colección de más de 5000 vectores, será necesario filtrar la colección previamente. Otra opción es limitar la cantidad de vectores que se vayan a mostrar. Para ello se puede usar el método `r colorize2(".limit")`, el cual permite especificar la cantidad de vectores que queremos ver (siempre que sean <= 5000; `r colorize("Fig.")` \@ref(fig:f82)).
:::
::::

## Creación de colecciones de vectores

Una colección de vectores se define a partir del método `r colorize2("ee.FeatureCollection")` y puede contener geometrías, vectores o inclusive otra colección de vectores. Una colección de vectores puede contener un solo vector o una sola geometría y sus elementos pueden tener o no atributos. Por ejemplo (`r colorize("Fig.")` \@ref(fig:f83), \@ref(fig:f84)): 

`r colorize("Ejercicio 15")`
...
```
```{r f83, echo = F, out.width="80%", fig.cap="Visualización de una colección de vectores con un único objeto de geometría de tipo punto."}
knitr::include_graphics("Img/ej15.png")
```

`r colorize("Ejercicio 16")` 
```{js echo = T}
...
```{r f84, echo = F, out.width="95%", fig.cap="Visualización de una colección de vectores con objetos con diferentes geometrías, así como su salida en la consola."}
knitr::include_graphics("Img/ej16.png")
```

## Visualización de colecciones de vectores
\index{Map.addLayer}
Al igual que los vectores, las colecciones de vectores se pueden agregar a la pantalla de mapa mediante la función `r colorize2("Map.addLayer")` (`r colorize("Fig.")` \@ref(fig:f85)).

`r colorize("Ejercicio 17")`
...
```
\index{paint}
Si queremos visualizar solo los contornos de los vectores se puede usar el siguiente código, el cual dibuja con el método `r colorize2(".paint")` los contornos de los vectores sobre una imagen vacía (`r colorize2("ee.Image")`), y luego se visualiza esa imagen (`r colorize("Fig.")` \@ref(fig:f85)): 

`r colorize("Ejercicio 17.2")`
...
```{r f85, echo = F, out.width="80%", fig.cap="Diferentes formas de visualizar colecciones de vectores."}
...
Para filtrar por atributos de los vectores se puede utilizar el método `r colorize2(".filterMetadata")`. De esta forma, todos los vectores de la colección que cumplan con una condición deseada serán seleccionados. Para usar `r colorize2(".filterMetadata")` se debe indicar dentro del paréntesis (separado por comas) el nombre entre comillas del atributo a evaluar y el tipo de comparación a hacer, de acuerdo con ciertos operadores lógicos. Los operadores lógicos aceptados en este método son: 
...
\index{filterDate}
\index{set}
El método para filtrar por fecha es `r colorize2(".filterDate")`, sin embargo, este solo se puede utilizar si los vectores tienen un atributo llamado 'system:time_start' y en ese atributo se encuentra la fecha en formato 'AAAA-MM-DD'. En caso de que los vectores no tengan ese atributo pero sí tengan un atributo de fecha, entonces será necesario renombrar dicho atributo de fecha a 'system:time_start', para ello se puede usar el método `r colorize2(".set")` y el método `r colorize2(".map")` (el método `r colorize2(".map")` se explicará con mayor detalle más adelante en esta sección).
...
Los filtros compuestos permiten combinar los filtros disponibles bajo la biblioteca de `r colorize2("ee.Filter")`. En el ejercicio 18.3 se mostrará cómo construir un filtro combinado para filtrar vectores que cumplan con todas las condiciones. El ejercicio consta de los siguientes pasos:

1.	El método `r colorize2(".filter")` es para indicar que vamos a realizar un filtro.
2.	`r colorize2("ee.Filter.and")` es para indicar que se deben cumplir todas las condiciones para filtrar.
3.	`r colorize2("ee.Filter.gte")` es para seleccionar mediante la condición mayor o igual a (*greater than or equal*).
4.	Después se utiliza una coma para indicar otro filtro.
5.	`r colorize2("ee.Filter.stringContains")` es un filtro para seleccionar de acuerdo a criterios de cadenas de texto que se encuentren como atributos de los vectores.
6.	Después se usa otra coma para indicar otro filtro.
7.	`r colorize2("ee.Filter.rangeContains")` es para seleccionar todo lo que se encuentre en un rango de valores (`r colorize("Fig.")` \@ref(fig:f86)).
    

`r colorize("Ejercicio 18.3")`
...
\index{randomPoints}
GEE ofrece la herramienta para generar una colección de puntos aleatorios en una región especificada, mediante el método `r colorize2(".randomPoints")`. En ella, especifica el área de interés y la cantidad de puntos deseados (`r colorize("Fig.")` \@ref(fig:f87)).

`r colorize("Ejercicio 19")`
...
\index{propertyNames}
Para extraer los nombres de los metadatos de una colección de vectores se usa el método `r colorize2(".propertyNames")`, mientras que para extraer algún metadato en particular de una colección de vectores se debe utilizar el método de `r colorize2(".get")`, indicando dentro de los paréntesis el nombre de la propiedad (`r colorize("Fig.")` \@ref(fig:f88)). 
...
```
```{r f88, echo = F, out.width="95%", fig.cap="Ejemplos de la salida en la consola de diferentes propiedades de los vectores que forman una colección de vectores de cuencas."}
...
\index{map}
\index{function}
Para aplicar una operación en todos los vectores de una colección se usa el método `r colorize2(".map")`.

:::: {.bluebox2 data-latex=""}
::: {.awesomeblock data-latex="{5pt}{\faLightbulb}{darkblue}"}
Para poder ejecutar el método `r colorize2(".map")` es necesario  definir antes una función (del usuario). Para definir una función es necesario tener en cuenta varias cosas: 
...
*	Usando el método `r colorize2(".map")` ejecutamos la función `bufferAleatorio` sobre todos los puntos aleatorios.

`r colorize("Ejercicio 21")`
\index{ceil}
\index{multiply}
\index{randomPoints}
```{js echo = T}
...
```{r f89, echo = F, out.width="95%", fig.cap="Visualización de varios buffers, así como la salida de la consola."}
...
*	Definimos una función del usuario (`function`) llamada 'areaDife' en la que el dato de entrada (insumo) será un vector (y el nombre genérico que le asignamos al insumo es ('cuenca')).
  ...
  -	Calculamos el área del vector (con el método `r colorize2(".area")`) y lo dividimos por 1 000 000 para convertir a km^2^.
  -	Con `r colorize2(".get")` extraemos el valor del atributo 'areasqkm' (el cual es una cadena de texto).
  -	Con `r colorize2(".parse")` convertimos el valor extraído anteriormente a un número del servidor (`r colorize2("ee.Number")`).
  -	Usando el método `r colorize2(".subtract")` restamos el área calculada con el método `r colorize2(".area")` menos el área reportada según el atributo del vector.
  -	Usando el método `return` especificamos que el vector sea devuelto con un nuevo atributo llamado 'diferencia' (el cual contiene el valor de la resta elevado al cuadrado usando el método `r colorize2(".pow")`).
*	Ejecutamos la función 'areaDife' sobre todos los vectores de la colección `cuencasHawaii`.
*	Usando el método `r colorize2(".reduceColumns")` declaramos que usaremos un reductor sobre toda la colección enfocándonos en un atributo de los vectores.
*	Con `r colorize2("ee.Reducer.mean")` declaramos que el reductor a utilizar será uno de promedio, por lo que el resultado será el  promedio del atributo 'diferencia'.
*	Con `r colorize2(".get")` extraemos el valor del reductor que por defecto se llama 'mean'.
*	Con `r colorize2(".sqrt")` calculamos la raíz cuadrada del promedio calculado.

\newpage
`r colorize("Ejercicio 22")`
...
var RECM = ee.Number(
  ...
  .get('mean')).sqrt();
```
```{r f810, echo = F, out.width="95%", fig.cap="Vista en la consola del error cuadrático medio calculado entre el área calculada directamente en GEE y el reportado por la fuente de datos utilizada."}
...
```
```{r f811, echo = F, out.width="95%", fig.cap="Visualización y salida de la consola de la consulta realizada sobre una imagen de variables climáticas utilizando la extensión espacial de una colección de vectores."}
...
Para rasterizar un vector se usa el método `r colorize2(".reduceToImage")`. Este método asigna bajo cada vector los píxeles del valor del atributo especificado. Para usar `r colorize2(".reduceToImage")` indicamos el atributo que corresponderá al valor de los píxeles del ráster como 'area'. Por último, con el método `r colorize2("ee.Reducer.first")` especificamos que el primer valor que encuentre en ese atributo sea asignado al valor del píxel (`r colorize("Fig.")` \@ref(fig:f812)).

`r colorize("Ejercicio 24")`
...
```{r f812, echo = F, out.width="95%", fig.cap="Visualización de la transformación de una colección de vectores a imagen."}
knitr::include_graphics("Img/ej24.png")
```

### Unión de colecciones de vectores {-}
\index{ee.Join}
Para unir diferentes colecciones de vectores mediante los valores de sus atributos se usan las funciones de `r colorize2("ee.Join")`. El resultado de estas funciones suele constar de una colección de vectores que contendrá los atributos de otra colección de vectores. Para determinar el tipo de relación entre los atributos de los vectores de ambas colecciones se utiliza un filtro (`r colorize2("ee.Filter")`).

Adicionalmente, se debe definir el tipo de unión a realizar entre los vectores. Los diferentes tipos de uniones se pueden consultar en la biblioteca `r colorize2("ee.Join")`. En el siguiente ejercicio realizaremos un tipo de unión espacial de tipo `r colorize2("saveALL")`, pero lo usaremos con dos filtros diferentes (uno de intersección y otro de distancia).

En este ejercicio se busca saber cuáles áreas protegidas se encuentran a menos de 10 km de una planta de energía y ver cuáles áreas protegidas tienen una planta de energía dentro de sus límites (`r colorize("Fig.")` \@ref(fig:f813)).

`r colorize("Ejercicio 25")`
...
```{r f813, echo = F, out.width="95%", fig.cap="Visualización y salida de la consola del resultado de una unión de dos colecciones de vectores mediante sus propiedades."}
...
Para exportar vectores es necesario usar la función `r colorize2("Export.table")` y especificar si va a ser para drive (`r colorize2(".toDrive")`) o para los **Assets** (`r colorize2(".toAsset")`). Sus argumentos en común son: 

* `Collection` (colección): obligatoriamente una colección de vectores `r colorize2("ee.FeatureCollection")`. Si se quiere exportar un solo vector se deberá convertir en una colección de un solo elemento. 
...
Los argumentos específicos para exportar a drive `r colorize2("Export.table.toDrive")` son: 
...
El argumento adicional para exportar a los **Assets** `r colorize2("Export.table.toAssets")` es 'AssetID', que corresponde el nombre que le pondremos al nuevo archivo (`r colorize("Fig.")` \@ref(fig:f814)).

`r colorize("Ejercicio 26")`
\index{Export.table.toAsset}
\index{Export.table.toDrive}
```{js echo = T}
// Determinamos un área de estudio con un polígono
var poligono = ee.Geometry.Polygon([-79.59, 7.00,
          -85.02, 8.14,
          -88.04, 10.68,
          -93.69, 9.44,
          -98.51, 4.69,
...
```{r f814, echo = F, out.width="95%", fig.cap="Visualización de la información a exportar, así como su salida en la pestaña de (ref:sub5) (tareas)."}
knitr::include_graphics("Img/ej26.png")
```

Es importante recordar que es necesario dar clic al botón de **Run** en la pestaña de **Tasks** (tareas) para que se lleve a cabo la exportación (`r colorize("Fig.")` \@ref(fig:f815)).

(ref:sub4) **Run**

```{r f815, echo = F, out.width="80%", fig.cap="Ubicación del botón (ref:sub4) para ejecutar una tarea de exportación."}
