...
```{r f21, echo = F, out.width="80%", fig.cap="Pantalla de inicio en la página de registro de GEE."}
...
```{r f22, echo = F, out.width="60%", fig.cap="Formulario de datos a llenar para registrarse en GEE."}
...
```{r f23, echo = F, out.width="60%", fig.cap="Ejemplo del correo de confirmación por parte de GEE para poder utilizar la API."}
...
Una vez registrados, se podrá acceder al material de todos los ejercicios entrando al siguiente enlace: [**https://code.earthengine.google.com/?accept_repo=users/JonathanVSV/GEE_manual**](https://code.earthengine.google.com/?accept_repo=users/JonathanVSV/GEE_manual){.uri}
...
```{r f24, echo = F, out.width="95%", fig.cap="Vista inicial de la API de GEE."}
...
```{r f25, echo = F, out.width="80%", fig.cap="Opciones disponibles dentro de la pestaña de Nuevo."}
...
```{r f26, echo = F, out.width="60%", fig.cap="Vista del panel de repositorios dentro de GEE."}
...
```{r f27, echo = F, out.width="60%", fig.cap="Vista del espacio de propietario."}
...
```{r f28, echo = F, out.width="60%", fig.cap="Ejemplo de la pestaña de documentación dentro de GEE."}
knitr::include_graphics("Img/docs.png")
```

```{r f29, echo = F, out.width="60%", fig.cap="Ejemplo de consulta de un método dentro de la sección de documentación."}
...
```{r f210, echo = F, out.width="80%", fig.cap="Carpeta de inicio dentro de la API."}
...
```{r f211, echo = F, out.width="60%", fig.cap="Vista de la sección de información del usuario."}
...
```{r f212, echo = F, out.width="70%", fig.cap="Vista de la pantalla de rutinas o scripts."}
knitr::include_graphics("Img/editor.png")
```

```{r f213, echo = F, out.width="50%", fig.cap="Ubicación del botón para obtener enlaces para compartir rutinas."}
...
```{r f214, echo = F, out.width="50%", fig.cap="Ubicación del botón para guardar el código."}
...
```{r f215, echo = F, out.width="50%", fig.cap="Ubicación del botón para correr el código."}
...
```{r f216, echo = F, out.width="95%", fig.cap="Vista de la barra de búsqueda."}
...
```{r f217, echo = F, out.width="95%", fig.cap="Vista de la pantalla de mapa, indicando las diferentes herramientas que se pueden utilizar en ella."}
knitr::include_graphics("Img/drawing.png")
```

1.  Dibujar y visualizar puntos, polígonos, líneas o rectángulos.
2.  Mostrar la información que se haya indicado mediante la función `r colorize2("Map.addLayer")`.
...
```{r f218, echo = F, out.width="95%", fig.cap="Ejemplo del uso del administrador de capas."}
...
La consola consiste en la pantalla de comunicación con el servidor (`r colorize("Fig.")` \@ref(fig:f219)). En ella se muestran los errores que se obtienen al correr un código o se puede mostrar la información indicada por la función `r colorize2("print")`.

```{r f219, echo = F, out.width="80%", fig.cap="Vista de la consola en GEE."}
...
```{r f220, echo = F, out.width="95%", fig.cap="Vista de la pestaña de la herramienta de Inspector."}
knitr::include_graphics("Img/inspector.png")
```

#### Tasks {.unnumbered}

En esta pestaña se muestran las tareas que se hayan exportado mediante la función `r colorize2("Export")` y permite correr el trabajo de exportación al sitio donde el usuario le haya indicado (drive, assets; `r colorize("Fig.")` \@ref(fig:f221)). Además, esta pestaña tiene otras funcionalidades:
...
```{r f221, echo = F, out.width="95%", fig.cap="Vista de la pestaña de tareas dentro de GEE."}
...
3.  Comúnmente en JavaScript se utiliza la notación *lowerCamelCase* para unir palabras (unir palabras sin espacio, pero cada una comenzando con su respectiva mayúscula), en lugar de guiones `-` o guion bajo `_`. Por ejemplo: `r colorize2("intervalMean")` o `r colorize2("updateMask")`.
...
::: {.bluebox2 data-latex=""}
::: {.awesomeblock data-latex="{5pt}{\\faLightbulb}{darkblue}"}
El término correcto para llamar a los procesos aplicados con el operador `.` es el de método (*method*), lo cual quiere decir que son procedimientos ligados a un tipo de objeto. Por ejemplo, en `r colorize2("ee.Image.updateMask")`, el método `r colorize2("updateMask")` es uno ligado a un objeto de tipo imagen (`r colorize2("ee.Image")`). Por el contrario, las funciones en un sentido estricto se aplican a un objeto mediante la sintaxis: `function(objeto){}`. Distintos tipos de objetos tienen diferentes métodos ligados a ellos.
:::
:::
