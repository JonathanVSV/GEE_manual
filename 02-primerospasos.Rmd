...
```{r, echo = F, out.width="90%", fig.cap="Pantalla de incio en la página de registro de GEE."}
...
```{r, echo = F, out.width="40%", fig.cap="Formulario de datos a llenar para registrarse en GEE."}
...
```{r, echo = F, out.width="60%", fig.cap="Ejemplo del correo de confirmación por parte de GEE para poder utilizar la API."}
...
```{r, echo = F, out.width="100%", fig.cap="Vista inicial de la API de GEE."}
...
```{r, echo = F, out.width="40%", fig.cap="Vista de la sección de información del usuario."}
...
```{r, echo = F, out.width="50%", fig.cap="Vista de la pantalla de rutinas o scripts."}
...
A continuación, está el botón`Run` que sirve para correr,de principio a fin, el código que se muestra en la pantalla de rutinas (Fig. 2.16). Al darle clic a este botón se envía el código a los servidores de Google y se realiza el procedimiento indicado.
...
2.	Siempre se debe cerrar cualquier comando con un `;` (aunque de no hacerlo el código funcionará igualmente; sin embargo, se recomienda utilizarlo para tener un código más claro y ordenado).
3.	Comúnmente en Javascript se utiliza la notación "lowerCamelCase" para unir palabras (unir palabras sin espacio pero cada una comenzando con su respectiva mayúscula) , en lugar de guiones `-` o guión bajo `_`. Por ejemplo: `intervalMean` o `updateMask`.
